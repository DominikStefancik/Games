...
def handle_collisions(
    ...
    for laser in lasers_group:
        ...
        if pygame.sprite.spritecollide(laser, asteroids_group, True):
            AnimatedAsteroidExplosion(
                all_sprites_group, asteroid_explosion_frames, laser.rect.midtop
            )
            laser.kill()
    ...
    if pygame.sprite.spritecollide(
        spaceship, asteroids_group, True, pygame.sprite.collide_mask
    ):
        spaceship.kill()
        damage_sound.play()
        return True
...
def display_score(surface, score_font):
    ...
    text_rectangle = text_surface.get_frect(
    ...
    pygame.draw.rect(
        surface, SCORE_TEXT_COLOR, text_rectangle.inflate(20, 20).move(0, -8), 5, 10
    )
