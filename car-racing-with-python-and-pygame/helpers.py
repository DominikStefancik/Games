...
def scale_image(image, scale_factor):
    new_size = round(image.get_width() * scale_factor), round(
...
def draw(surface, images, computer_car, player_car, game_info, font, window_height):
    ...
    player_car_velocity_text = font.render(
        f"Velocity: {round(player_car.velocity, 1)}px/s", 1, (255, 255, 255)
    )
    surface.blit(
        player_car_velocity_text,
        (10, window_height - player_car_velocity_text.get_height() - 10),
    )
...
def render_text_center(surface, font, text):
    ...
    surface.blit(
        text_render,
        (
            surface.get_width() / 2 - text_render.get_width() / 2,
            surface.get_height() / 2 - text_render.get_height() / 2 - 25,
        ),
    )
