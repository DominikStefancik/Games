...
#[test]
fn snake_moves_when_game_is_playing() {
    ...
    let resumed_head_position = {
        let world = test_app.app.world_mut();
        get_head_position(world)
    };

    assert_ne!(
        resumed_head_position.column, initial_head_position.column,
        "Head should move while the game is playing"
    );
}

#[test]
fn snake_does_not_move_when_game_is_paused() {
    let mut test_app = TestApp::new();
    ...
    let paused_head_position = {
        let world = test_app.app.world_mut();
        get_head_position(world)
    };

    assert_eq!(
        paused_head_position.column, initial_head_position.column,
        "Head should not move while the game is paused"
    );
    assert_eq!(
        paused_head_position.row, initial_head_position.row,
        "Head should not move while the game is paused"
    );
}
