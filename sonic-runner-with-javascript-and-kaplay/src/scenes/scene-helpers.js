...
export const createBackgroundPieces = () => {
...
export const createPlatformPieces = () => {
  ...
  return platformsPieces;
};

// switches the background components,
// so the "first" component will become the "second" and the "second" will become the "first"
export const switchBackgroundPieces = (backgroundPieces) => {
...
// switches the platforms components,
// so the "first" component will become the "second" and the "second" will become the "first"
export const switchPlatformPieces = (platformsPieces, speed) => {
  // the "second" platform piece has gone far to left, outside of the bounds of a canvas
  // we can take the "first" first platform piece and put it behind
  if (platformsPieces[1].pos.x < 0) {
    // what we do here is to replace the order of the platform components
    // the "first" component will become the "second" and the "second" will become the "first"
  ...
  // "move" method allows you to move a game object according to a specific velocity
  platformsPieces[0].move(-speed, 0);
  platformsPieces[1].moveTo(
    platformsPieces[0].pos.x + PLATFORMS_SPRITE_WIDTH * PLATFORMS_SPRITE_SCALE,
    450,
  );
};
